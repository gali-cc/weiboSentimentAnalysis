from weiboanalysis.dao import commentDao
import jieba
import re
import pandas as pd


def save_word_fre_toCsv(sorted_word_fre_list, file_path='weiboanalysis/fenci/comment_fre.csv'):
    """
...
def cut_comment():
    ...
    对评论文本进行分词
    """
    comment_list = [comment[1] for comment in commentDao.getAllComment()]
    all_commentStr = ''.join(comment_list)
    seg_list = jieba.cut(all_commentStr)  # 默认是精确模式
